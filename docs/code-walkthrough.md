...
It validates pipe creation and `fork()`, then calls `fork()`. Before forking, it converts the configured handler path to an executable path so a later directory change cannot make a relative handler unresolvable. The child duplicates the read end of the input pipe to standard input and the write end of the output pipe to standard output, changes directory to the script directory, builds CGI/1.1 variables from `Request`, the resolved URL script path, and the selected `ServerConfig`, then calls `execve()`. `SCRIPT_NAME` is the URL script path such as `/test.sh`, while a remaining URL suffix such as `/extra` becomes `PATH_INFO`; `SERVER_NAME` and `SERVER_PORT` come from the listener-selected server. If setup or `execve()` fails, the child exits rather than returning to the server loop. The parent closes the child-only pipe ends, stores the child pid and remaining pipe fds in `CgiJob`, and marks those parent pipe ends non-blocking.
