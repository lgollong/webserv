...
| `SessionStore` | Process-local opaque sessions and cookie helpers. | Owned by `Worker`; used by the reference-mock `/session` bonus route. |
...
`Worker` uses the error-status overload. On a `-1` result, it receives `400` for malformed syntax/framing, `413` for a body over the parser limit, or `431` for a header limit. It clears the unprocessable input, looks up a configured error page for the connection's selected server, and uses that regular file's body and MIME type when it is readable. Otherwise it asks `Http::defaultErrorResponse()` for a matching HTML `Response`. It then serializes the result through `Http::build()`, marks `Connection::close_after_write`, and switches the fd to `POLLOUT`. This prevents malformed data from leaving the connection waiting in `POLLIN` forever while still returning a complete HTTP response.
...
Current-state note: request-line, header syntax/framing, fixed-limit `Content-Length` and chunked-body assembly, selected-server body-size policy, and mock-backed custom error-page responses are implemented. Parsed configuration remains incomplete.
...
On the current branch, [`Config`](../srcs/Config.cpp) explicitly constructs a reference in-memory `ServerConfig` model rather than parsing the supplied file. The fixture includes two server/listener records and routes for static content, `.sh` CGI through `/bin/sh`, directly executable `.cgi` CGI, autoindex/index, uploads, redirects, and a GET-only cookie-session demonstration. `Config::route(server_index, request)` resolves the longest matching location on the server selected at accept time, then derives `Route::is_cgi`, optional `cgi_handler`, URL `cgi_script_name`, and route-root `cgi_script_path` from that route's extension-to-handler map. A non-empty route method set rejects absent methods as `405 Method Not Allowed` with an `Allow` header before any downstream handler. A route-approved `DELETE` uses the same route-relative resolver as static GET, removes only a regular file with `std::remove`, and returns `204 No Content`; it never enters redirect or CGI dispatch. A `session_demo` route looks up or creates a process-local session, advances its visible counter, and emits `Set-Cookie` only for a newly created session. A route with a 3xx `redirect_status` and non-empty `redirect_target` is otherwise serialized immediately as an empty redirect response with a `Location` header. `make config-model-test` verifies the model contract; `make cookie-session-test` verifies the bonus flow; and `make connection-lifecycle-test` verifies listener-specific routing, method policy, deletion, and `/redirect` over a persistent client connection. #4 will replace only fixture construction with parsing; the session demonstration flag is intentionally mock-only. See [Runtime Configuration Model](configuration-model.md) for the fields and handoff boundary.
...
Current-state note: route-relative file serving, MIME detection, lexical traversal rejection, directory index/autoindex, configured route-method enforcement, permitted static DELETE, route-authorized uploads, configured redirect dispatch, and mock-backed server error pages are implemented. Parsed configuration remains incomplete.
...
Mocked `/cgi/test.sh` and `/cgi/test.cgi` routes take the CGI branch instead of `StaticFile`. The first is interpreted by `/bin/sh`; the second is a small direct executable built from a C++98 fixture, so the mock demonstrates both handler forms without an external runtime.
...
It validates pipe creation and `fork()`, then calls `fork()`. Before forking, it resolves an optional handler path and canonicalizes the requested script path, rejecting a script outside the route root. The child duplicates the read end of the input pipe to standard input and the write end of the output pipe to standard output, changes directory to the script directory, builds CGI/1.1 variables from `Request`, the resolved URL script path, and the selected `ServerConfig`, then calls `execve()` with either `handler script` or the directly executable script. `SCRIPT_NAME` is the URL script path such as `/cgi/test.sh`, while a remaining URL suffix such as `/extra` becomes `PATH_INFO`; `SERVER_NAME` and `SERVER_PORT` come from the listener-selected server. If setup or `execve()` fails, the child exits rather than returning to the server loop. The parent closes the child-only pipe ends, stores the child pid and remaining pipe fds in `CgiJob`, and marks those parent pipe ends non-blocking.
...
Current-state note: access logging is still a stub. Managed socket and pipe error paths now clean up their affected connection, malformed requests receive a response before their connection closes, inactive clients expire after 30 seconds, and CGI children time out/reap without a blocking wait. `make resilience-test` exercises the deadline and cleanup flows end to end, while `make event-loop-stress-test` covers real closed-descriptor `POLLNVAL`, controlled client/CGI error dispatch, and partial-write resumption with a constrained non-blocking socket pair. Broader network-load coverage still needs work.
